...
class Table {
    ...
    constructor(row, col, container) {
        this.canvas = new fabric.fabric.StaticCanvas(container);
        this.cellWidth = 120;
        this.cellHeight = 40;
        this.row = row;
        this.col = col;
    ...
    initMaskTable() {
        ...
        for (let i = 0; i <= this.row; i++) {
            let horizontalLine = new fabric.fabric.Line([0, i * this.cellHeight, totalWidth, i * this.cellHeight], {
                stroke: "black",
                strokeWidth: 1,
            });
    ...
    mergeCells(startRow, startCol, endRow, endCol) {
        ...
        var text = new fabric.fabric.Textbox("Hello", {
            left: rect.left,
            top: rect.top,
            width: rect.width,
            height: rect.height,
